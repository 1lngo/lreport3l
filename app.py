...
def main():
    ...
    with tab_kronos:
        ...
        **Kronos** 是专门用于金融时序预测的基础模型 (Foundation Model)。
        ...
        # ========== 自动加载默认CSV文件 ==========
        default_csv_path = os.path.join(_THIS_DIR, "a_share_mock_2.csv")
        if os.path.exists(default_csv_path) and 'kronos_df' not in st.session_state:
            with st.spinner("🔄 正在自动加载默认数据文件..."):
                try:
                    df_auto = pd.read_csv(default_csv_path)
                    st.session_state['kronos_df'] = df_auto
                    st.session_state['kronos_source'] = "默认: a_share_mock_2.csv"
                    st.info("✅ 已自动加载默认数据文件: a_share_mock_2.csv")
                except Exception as e:
                    st.warning(f"自动加载失败: {e}")
        ...
        if 'kronos_df' in st.session_state:
            ...
            if missing_cols:
                ...
                if len(df) < k_lookback + k_pred_len:
                    ...
                    # ========== 自动预测（如果是默认加载的数据） ==========
                    auto_run = ('kronos_source' in st.session_state and
                                '默认' in st.session_state['kronos_source'] and
                                'kronos_prediction' not in st.session_state)

                    if auto_run:
                        st.info("🤖 正在自动运行 Kronos 预测...")
                        # 强制使用 kronos-small
                    ...
                    # Run prediction
                    if st.button("🚀 运行 Kronos 预测", type="primary", key="run_kronos_tab6") or auto_run:
                        if not KRONOS_AVAILABLE:
